...
require 'fog/openstack/baremetal/models/port'

module Fog
  module OpenStack
    class Baremetal
      class Ports < Fog::OpenStack::Collection
        model Fog::OpenStack::Baremetal::Port
        ...
        def method_missing(method_sym, *arguments, &block)
          if method_sym.to_s =~ /^find_by_(.*)$/
            load(service.list_ports_detailed(Regexp.last_match(1) => arguments.first).body['ports'])
          else
            super
          end
        end
