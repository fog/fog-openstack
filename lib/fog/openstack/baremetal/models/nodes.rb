...
require 'fog/openstack/baremetal/models/node'

module Fog
  module OpenStack
    class Baremetal
      class Nodes < Fog::OpenStack::Collection
        model Fog::OpenStack::Baremetal::Node
        ...
        def method_missing(method_sym, *arguments, &block)
          if method_sym.to_s =~ /^find_by_(.*)$/
            load(service.list_nodes_detailed(Regexp.last_match(1) => arguments.first).body['nodes'])
          else
            super
          end
        end
