  ...
  it 'CRUD & list shares' do
    VCR.use_cassette('share_crud') do
      ...
      begin
        ...
        _(example_share.status).must_equal 'creating'
        example_id = example_share.id

        # update display description
        example_share.update(:display_description => share_updated_description)
        _(example_share.reload.description).must_equal share_updated_description

        # get by ID
        example_share_by_id = @service.shares.find_by_id example_id
        _(example_share_by_id).wont_equal nil
        _(example_share_by_id.name).must_equal share_name

        # get by filtering list by name
        shares = @service.shares.all(:name => share_name)
        _(shares.length).must_equal 1
        _(shares.first.id).must_equal example_id
      ensure
        # delete the share(s)
        @service.shares.all(:name => share_name).each(&:destroy)
        # check delete action
        _(@service.shares.all(:name => share_name).length).must_equal 0
  ...
  it "CRUD & list share_networks" do
    VCR.use_cassette('share_net_crud') do
      ...
      begin
        ...
        _(share_network.description).must_equal share_net_description
        share_net_id = share_network.id

        # update description
        share_network.update(:description => share_net_updated_description)
        _(share_network.reload.description).must_equal share_net_updated_description

        # get by ID
        share_net_by_id = @service.networks.find_by_id share_net_id
        _(share_net_by_id).wont_equal nil
        _(share_net_by_id.name).must_equal share_net_name

        # get by filtering list by name
        share_nets = @service.networks.all(:name => share_net_name)
        _(share_nets.length).must_equal 1
        _(share_nets.first.id).must_equal share_net_id
      ensure
        # delete the share network
        @service.networks.all(:name => share_net_name).each(&:destroy)
        # check delete action
        _(@service.networks.all(:name => share_net_name).length).must_equal 0
      end
    end
  end
  ...
  it 'CRUD & list share_snapshots' do
    VCR.use_cassette('share_snap_crud') do
      ...
      begin
        ...
        _(snap.name).must_equal snap_name
        snap_id = snap.id
        snap.wait_for { ready? }

        # get by ID
        snap_by_id = @service.snapshots.find_by_id snap_id
        _(snap_by_id).wont_equal nil
        _(snap_by_id.name).must_equal snap_name

        # update name via display_name
        snap_by_id.update(:display_name => snap_updated_name)
        _(snap_by_id.reload.name).must_equal snap_updated_name

        # get by filtering list by name
        snaps = @service.snapshots.all(:name => snap_updated_name)
        _(snaps.length).must_equal 1
        _(snaps.first.id).must_equal snap_id
      ensure
        ...
        @service.snapshots.all(:name => snap_updated_name).each do |s|
          _(s.status).must_equal 'deleting'
        end
  ...
  it 'acts on shares' do
    VCR.use_cassette('share_actions') do
      ...
      begin
        ...
        # modify share sizes
        _(share.size).must_equal share_size_small
        share.extend(share_size_big)
        share.wait_for { ready? }
        _(share.size).must_equal share_size_big
        share.shrink(share_size_small)
        share.wait_for { ready? }
        _(share.size).must_equal share_size_small

        # modify share access
        _(share.access_rules.length).must_equal 0
        ...
        _(new_rule.access_type).must_equal share_access_type
        _(new_rule.id).must_equal access_rule.id
