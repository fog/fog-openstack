...
describe Fog::OpenStack::Monitoring do
  ...
  it 'list measurements by name and start_time' do
    VCR.use_cassette('metric_measurement_list') do
      ...
      _(measurement_list).wont_be_nil
      _(measurement_list).wont_be_empty

      # should return an empty list
      measurement_list_empty = @service.measurements.find(:name       => 'non.existing.metric_name',
                                                          :start_time => '2016-04-01T14:43:00Z')

      _(measurement_list_empty).must_be_empty
    end
  end
  ...
  it 'find statistics specified by name, start_time and statistics' do
    VCR.use_cassette('statistics_list') do
      ...
      _(statistics_list).wont_be_nil
      _(statistics_list).wont_be_empty
    end
  end
  ...
  it 'notification methods crud test' do
    VCR.use_cassette('notification-methods_crud') do
      begin
        ...
        _(notification_methods_all).wont_be_nil
        _(notification_methods_all).wont_be_empty

        # find a notification method by id
        notification_method_by_id = @service.notification_methods.find_by_id(notification_method.id)

        _(notification_method).must_equal notification_method_by_id
        ...
        _(notification_method.address).must_equal 'notification_methods@example.com'
        _(notification_method.period).must_equal 0

        # Delete the notification method and make sure it is no longer found in the list
        notification_method.destroy
        _((@service.notification_methods.all.include? notification_method)).must_equal false
  ...
  it 'alarm definitions crud test' do
    VCR.use_cassette('alarm_definition_crud') do
      begin
        ...
        _(alarm_definitions_all).wont_be_nil
        _(alarm_definitions_all).wont_be_empty

        # find an alarm-definition by id
        alarm_definition_by_id = @service.alarm_definitions.find_by_id(alarm_definition.id)

        _(alarm_definition.id).must_equal alarm_definition_by_id.id
        _(alarm_definition.name).must_equal alarm_definition_by_id.name
        _(alarm_definition.expression).must_equal alarm_definition_by_id.expression
        _(alarm_definition.deterministic).must_equal false
        ...
        _(alarm_definition_replaced.id).must_equal alarm_definition.id
        _(alarm_definition_replaced.name).must_equal 'CPU percent greater than 15'
        _(alarm_definition_replaced.description).must_equal 'Replaced alarm-definition expression'
        _(alarm_definition_replaced.expression).must_equal '(avg(cpu.user_perc{hostname=devstack}) > 15)'
        #
        ...
        _(alarm_definition.id).must_equal alarm_definition_updated.id
        _(alarm_definition_updated.description).must_equal 'An updated alarm-definition.'

        # delete alarm definition
        alarm_definition.destroy

        # ensure the alarm definition does not exist any more
        _((@service.alarm_definitions.all.include? alarm_definition)).must_equal false
  ...
  it 'alarm crud test' do
    VCR.use_cassette('alarm_crud') do
      begin
        ...
        _(alarms_all).wont_be_nil
        _(alarms_all).wont_be_empty

        # list all alarms in ALARM state
        alarms_all_state_filter = @service.alarms.all(:state => 'OK')

        _(alarms_all_state_filter).wont_be_nil
        _(alarms_all_state_filter).wont_be_empty
        ...
        _(alarm_by_name.id).must_equal alarm_by_id.id
        _(alarm_by_name.state).must_equal alarm_by_id.state
        _(alarm_by_name.lifecycle_state).must_equal alarm_by_id.lifecycle_state
        ...
        _(alarm_by_id.state).must_equal 'ALARM'
        _(alarm_by_id.lifecycle_state).must_equal 'OPEN'
        _(alarm_by_id.link).must_equal 'http://pagerduty.com/'

        # check the link of an alarm before patching
        _(alarm_by_id.link).wont_equal 'http://somesite.com/this-alarm-info'
        ...
        _(alarm_by_id.link).must_equal 'http://somesite.com/this-alarm-info'

        # list alarm state history for all alarms but limit history to 5
        alarm_state_history_all = @service.alarm_states.all(:limit => 5)

        _(alarm_state_history_all).wont_be_nil
        _(alarm_state_history_all.length).must_equal 5

        # list alarm state history
        alarm_state_history_for_alarm = @service.alarm_states.list_alarm_state_history(alarm_id)

        _(alarm_state_history_for_alarm).wont_be_nil
        _(alarm_state_history_for_alarm).wont_be_empty

        # delete an existing alarm
        alarm_by_id.destroy

        _((@service.alarms.all.include? alarm_by_id)).must_equal false
  ...
  it 'list alarm state history for all alarms' do
    VCR.use_cassette('alarm_state_history_all') do
      ...
      _(alarm_state_history_all).wont_be_nil
      _(alarm_state_history_all).wont_be_empty
  ...
  it 'list dimension values' do
    VCR.use_cassette('list_dimension_values') do
      ...
      _(dimension_values_by_key).wont_be_nil
      _(dimension_values_by_key.dimension_name).must_equal 'hostname'
      _(dimension_values_by_key.id).wont_be_nil
      _(dimension_values_by_key.values).wont_be_empty
    end
  end
  ...
  it 'list notification method types' do
    VCR.use_cassette('list_notification_method_types') do
      ...
      _(notification_method_types).wont_be_nil
      _(notification_method_types).wont_be_empty

      _(notification_method_types).must_include :EMAIL
      _(notification_method_types).must_include :PAGERDUTY
      _(notification_method_types).must_include :WEBHOOK
    end
  end
