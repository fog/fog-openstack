...
[Fog::Volume::OpenStack,
 ...
 Fog::Volume::OpenStack::V2].delete_if { |_class| ENV['TEST_CLASS'] && ENV['TEST_CLASS'] != _class.name }.each do |service_class|
  RSpec.describe service_class do
    ...
    before :all do
      vcr_directory = 'spec/fog/volume' if service_class == Fog::Volume::OpenStack

      vcr_directory = 'spec/fog/volume_v1' if service_class == Fog::Volume::OpenStack::V1

      vcr_directory = 'spec/fog/volume_v2' if service_class == Fog::Volume::OpenStack::V2
    ...
    def setup_test_object(options)
      ...
      case type
        ...
        when :snapshot
          ...
          # if this fails, cleanup this object (it was left over from a failed test run)
          expect(@service.snapshots.all(@name_param => snapshot_name).length).to be(0)
    ...
    def cleanup_test_object(collection, id)
      ...
      Fog.wait_for do
        begin
          ...
          object.nil? || (['available', 'error', 'awaiting-transfer'].include? object.status.downcase)
        end
      end

      object = collection.get(id)

      if object && object.status.downcase == 'awaiting-transfer'
    ...
    it 'can extend volumes' do
      VCR.use_cassette('volume_extend') do
        begin
          ...
          volume.wait_for {
            self.status == 'error_extending' or (ready? and size == volume_size_large)
          }
          expect(volume.status).to_not eq 'error_extending'
    ...
    it 'can create, update and delete volume snapshots' do
      VCR.use_cassette('volume_snapshot_and_delete') do
        begin
          ...
          # cleanup volume
          snapshot.destroy if snapshot rescue false
          cleanup_test_object(@service.volumes, volume.id) if volume
        end
      end
    end
    ...
    it 'can create, update and delete volume metadata' do
      VCR.use_cassette('volume_metadata_crud') do
        begin
          ...
          expect(check_metadata['some_metadata']).to eq 'this is meta'
          expect(check_metadata['more_metadata']).to eq 'even more meta'
          ...
          updated_volume = @service.volumes.get(volume.id)
          check_metadata = updated_volume.metadata
          expect(check_metadata.size).to be 3
          expect(check_metadata['some_metadata']).to eq 'this is updated'
          expect(check_metadata['more_metadata']).to eq 'even more meta'
          expect(check_metadata['new_metadata']).to eq 'this is new'
          ...
          check_metadata = updated_volume.metadata
          expect(check_metadata.size).to be 2
          expect(check_metadata['some_metadata']).to eq 'this is updated again'
          expect(check_metadata['newer_metadata']).to eq 'this is newer'
