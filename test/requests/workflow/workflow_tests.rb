...
describe "Fog::OpenStack::Workflow | Workflow requests" do
  ...
  describe "success" do
    it "#create_execution" do
      _(@workflow.create_execution(@workflow_name, @input).status).must_equal 201
    end

    it "#get_execution" do
      response = @workflow.get_execution(@execution_id)
      _(response.status).must_equal 200
      _(response.body).must_match_schema(@get_execution_response)
    end

    it "#list_executions" do
      _(@workflow.list_executions.status).must_equal 200
    end
    ...
    it "#create_action_execution" do
      ...
      _(@workflow.create_action_execution(action_name, @input).status).must_equal 201
    end
    ...
    it "#list_action_executions" do
      _(@workflow.list_action_executions.status).must_equal 200
    end
    ...
    it "#create_workbook" do
      _(@workflow.create_workbook(@workbook_sample).status).must_equal 201
    end

    it "#get_workbook" do
      response = @workflow.get_workbook(@workbook_sample[:name])
      _(response.status).must_equal 200
      _(response.body).must_match_schema(@workbook_sample)
    end

    it "#list_workbooks" do
      _(@workflow.list_workbooks.status).must_equal 200
    end
    ...
    it "#create_workflow" do
      _(@workflow.create_workflow(@workflow_sample).status).must_equal 201
    end

    it "#get_workflow" do
      response = @workflow.get_workflow(@workflow_sample[:name])
      _(response.status).must_equal 200
      _(response.body).must_match_schema(@workflow_sample)
    end

    it "#list_workflows" do
      _(@workflow.list_workflows.status).must_equal 200
    end
    ...
    it "#create_action" do
      _(@workflow.create_action(@action_sample).status).must_equal 201
    end

    it "#get_action" do
      response = @workflow.get_action("action1")
      _(response.status).must_equal 200
      _(response.body).must_match_schema(@action_sample)
    end

    it "#list_actions" do
      _(@workflow.list_actions.status).must_equal 200
    end
    ...
    it "#get_task" do
      ...
      _(response.status).must_equal 200
      _(response.body).must_match_schema(@task_sample)
    end
    ...
    it "#create_cron_trigger" do
      _(@workflow.create_cron_trigger("cron1", 1, "input").status).must_equal 201
    end

    it "#get_cron_trigger" do
      response = @workflow.get_cron_trigger("cron1")
      _(response.status).must_equal 200
      _(response.body).must_match_schema(@cron_trigger_sample)
    end

    it "#list_cron_triggers" do
      _(@workflow.list_cron_triggers.status).must_equal 200
    end
    ...
    it "#create_environment" do
      _(@workflow.create_environment(@environment_sample).status).must_equal 201
    end

    it "#get_environment" do
      response = @workflow.get_environment("environment1")
      _(response.status).must_equal 200
      _(response.body).must_match_schema(@environment_sample)
    end

    it "#list_environments" do
      _(@workflow.list_environments.status).must_equal 200
    end
