...
describe "Fog::OpenStack::Network | subnet requests" do
  ...
  describe "success" do
    ...
    it "#create_subnet" do
      _(@subnet).must_match_schema('subnet' => @subnet_format)
    end

    it "#list_subnet" do
      _(network.list_subnets.body).must_match_schema('subnets' => [@subnet_format])
    end

    it "#get_subnet" do
      subnet_id = network.subnets.all.first.id
      _(network.get_subnet(subnet_id).body).must_match_schema('subnet' => @subnet_format)
    end
    ...
    it "#update_subnet" do
      ...
      _(network.update_subnet(subnet_id, attributes).body).must_match_schema('subnet' => @subnet_format)
    end

    it "#delete_subnet" do
      subnet_id = network.subnets.all.first.id
      _(network.delete_subnet(subnet_id).status).must_equal 204
    end
  end
