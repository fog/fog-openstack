...
describe "Fog::OpenStack::Compute | address requests" do
  ...
  describe "success" do
    ...
    it "#allocate_address" do
      _(self.class.data).must_match_schema("floating_ip" => address_format)
    end

    it "#list_all_addresses" do
      _(compute.list_all_addresses.body).
        must_match_schema("floating_ips" => [address_format])
    end

    it "#get_address(address_id)" do
      _(compute.get_address(address_id).body).
        must_match_schema("floating_ip" => address_format)
    end

    it "#list_address_pools" do
      _(compute.list_address_pools.body).
        must_match_schema("floating_ip_pools" => [address_pools_format])
    end

    it "#associate_address(server_id, ip_address)" do
      _(compute.associate_address(self.class.server_id, address_ip).body).must_equal ""
    end

    it "#disassociate_address(server_id, ip_address)" do
      _(compute.disassociate_address(self.class.server_id, address_ip).body).must_equal ""
    end

    it "#release_address(ip_address)" do
      _(compute.release_address(address_id).status).must_equal 202
    end
  end
end
